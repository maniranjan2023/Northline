...
def upsert_and_describe(
    ...
    *,
    ...
    label = ATTRIBUTE_LABELS.get(clean_key, clean_key.replace("_", " ").title())
    return {
...
def filter_semantic_memories(memories: list[str], profile: dict[str, str]) -> list[str]:
    """Drop stale dietary Mem0 facts when structured food_preference exists."""
    ...
    for memory in memories:
        lower = memory.lower()
        if any(term in lower for term in DIETARY_TERMS) and food_pref not in lower:
            continue
