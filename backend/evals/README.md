# Northline — Evaluation Framework
...
> "Northline runs 6 agents and 3 MCP tools. A single bad release can leak PII, route a greeting into a full graph run, or forget a user's vegetarian preference. We run **13 evals** in three schedules: **CI on every PR** (guardrails + router, ~2 min), **nightly** (5 DeepEval agent metrics on real trip planning), and **weekly** (5 DeepEval conversation metrics on follow-ups and Mem0). Results append to Markdown logs so we can debug without a dashboard during development."
...
## Feedback-driven proposed cases

Thumbs-down feedback in Streamlit is attached to its LangSmith trace. The trace is analyzed and a draft case is saved under `datasets/proposed/` with:

- user request, response, and feedback comment
- observed tools and trace evidence
- suggested component to review
- target golden dataset and draft test shape

The same thumbs-down comment also creates or updates a **candidate lesson** in the PostgreSQL Lesson Book. After three similar observations, the candidate promotes to an active lesson that can guide future planning.

Drafts require review before they are copied into an approved golden dataset.

```powershell
python -m evals.helpers.trace_to_golden --collect-negative
```

Full workflow: [`docs/SELF_IMPROVEMENT.md`](../docs/SELF_IMPROVEMENT.md)
