...
def build_travel_graph(llm, memory_manager: MemoryManager, checkpointer: PostgresSaver):
    """
    Graph flow:
    START → retrieve_memory → planner → research → hotel → flight
          → activity → final_response → store_memory → END
    """
    ...
    graph.add_edge("retrieve_memory", "planner_agent")
    ...
    graph.add_edge("final_response_agent", "store_memory")
